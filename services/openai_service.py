    ...
    if message.attachments:
        for attachment in message.attachments:
            if re.search(r"\.(png|jpeg|jpg|gif|webp)$", attachment.filename):
                ...
                r"\.(c|cpp|csv|docx|html|java|json|md|pdf|php|pptx|py|py|rb|tex|txt)$",
                attachment.filename,
            ):
                # ファイルの内容を一時ファイルに保存
    ...
    for index, message_for_history in enumerate(messages_for_history):
        ...
        while retry_count < 5:
            try:
                ...
                if re.search(
                    "Can't add messages to thread_.* while a run .* is active.", str(e)
                ):
                    time.sleep(2)
                    retry_count += 1
                    continue
    ...
    try:
        ...
        while True:
            retrieve_run = client.beta.threads.runs.retrieve(
                thread_id=thread_id, run_id=create_run.id
            )
            ...
            if retrieve_run.status in END_ACTIONS:
                ...
                for index, tool_call in enumerate(
                    retrieve_run.required_action.submit_tool_outputs.tool_calls
                ):
                ...
                client.beta.threads.runs.submit_tool_outputs(
                    thread_id=thread_id, run_id=create_run.id, tool_outputs=tool_outputs
                )
            time.sleep(1)
        ...
        while True:
            retrieve_run = client.beta.threads.runs.retrieve(
                thread_id=thread_id, run_id=cancel_run.id
            )
            print(retrieve_run.status)
            if retrieve_run.status in END_ACTIONS:
                break
